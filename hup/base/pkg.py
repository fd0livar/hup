...
import importlib
import pkgutil
from hup.base import otree, mapping, stack
from hup.typing import check
from hup.typing import Any, OptStr, StrList, OptDictOfKeyOps, ClassInfo
from hup.typing import Optional, Module, Function
...
def get_root_name(fqn: str) -> str:
    root = []
    for token in fqn.split('.'):
        root.append(token)
        name = '.'.join(root)
        file = get_module(name).__file__
        if file:
            break
    if not file:
        raise ModuleNotFoundError() # TODO
    return name
