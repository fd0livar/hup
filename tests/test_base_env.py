...
class TestEnv(test.ModuleTest):
    ...
    def is_dir_valid(
        ...
        if dirname in self.pkg_dirs: # Check package dir
            # TODO (patrick.michl@frootlab.org): Check if root package name is in
            # str(path)
            return True
    ...
    def test_update_vars(self) -> None:
        vars_exist = hasattr(env, 'cache')
        try:
            if vars_exist:
                prev_vars = getattr(env, 'cache').copy()
            env.update_vars(pkgname=env.__name__)
            new_vars = env.get_vars(pkgname=env.__name__)
            self.assertEqual(new_vars.get('copyright'), env.__copyright__)
            self.assertEqual(new_vars.get('email'), env.__email__)
            self.assertEqual(new_vars.get('license'), env.__license__)
        finally:
            if vars_exist:
                setattr(env, 'cache', prev_vars)
            else:
                delattr(env, 'cache')
