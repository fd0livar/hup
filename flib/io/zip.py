...
__maintainer__ = 'Frootlab Developers <frootlab@gmail.com>'
...
from flib.io import abc
from flib.typing import BinaryFileLike, BytesLike, OptBytes, OptStr
from flib.typing import OptPathLike
from flib.typing import PathLike, PathLikeList, ErrMeta, ErrType, ErrStack
from flib.typing import FileLike, StrList, OptPath, AnyOp
...
class File:
    ...
    def get_file_accessor(self, path: PathLike) -> abc.Connector:
        ...
        Returns:
            :class:`File accessor <flib.io.abc.Connector>` to ZipFile
            member.

        """
        def wrap_open(path: PathLike) -> AnyOp:
            def wrapped_open(
                    obj: abc.Connector, *args: Any, **kwds: Any) -> FileLike:
                return self.open(path, *args, **kwds)
            return wrapped_open

        return type( # pylint: disable=E0110
            'Connector', (abc.Connector, ), {
            'name': str(path),
            'open': wrap_open(path)})()
