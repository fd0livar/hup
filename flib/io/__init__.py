...
from flib.base.abc import Proxy
from flib.errors import PullError, PushError, ConnectError, DisconnectError
from flib.typing import Iterator, PathLike, FileLike, OptStr
from flib.typing import ErrMeta, ErrType, ErrStack, BinaryFileLike
from flib.typing import TextFileLike
from flib.io.abc import Connector

#
# Structural Types and Class Infos
#

FileRef = Union[IO[Any], os.PathLike, Connector]
FileRefClassInfo = (
    str, io.BufferedIOBase, io.TextIOBase, os.PathLike, Connector)
...
class FileInfo:
    ...
    Args:
        file: :term:`File reference` to a :term:`file object`. The reference can
            ...
            :class:`~flib.io.abc.Connector` or an opened file object in
...
class FileConnector:
    ...
    def open(self, *args: Any, **kwds: Any) -> FileLike:
        ...
        if isinstance(ref, (str, Path)):
            return self._open_path(ref, *args, **kwds)
        if isinstance(ref, io.TextIOBase):
            return self._open_textfile(ref, *args, **kwds)
        if isinstance(ref, io.BufferedIOBase):
            return self._open_binary(ref, *args, **kwds)
        if isinstance(ref, Connector):
            return self._open_connector(ref, *args, **kwds)
    ...
    def _open_textfile(self, file: TextFileLike, mode: str = 'r') -> FileLike:
    ...
    def _open_binary(self, file: BinaryFileLike, mode: str = 'r') -> FileLike:
        # Check reading / writing mode
        file_mode = getattr(file, 'mode', None)
        if file_mode and file_mode not in mode:
            file_name = getattr(file, 'name', '?')
    ...
    def _open_connector(
            self, ref: Connector, *args: Any, **kwds: Any) -> FileLike:
...
class FileProxy(Proxy):
    ...
    Args:
        ...
        mode: String, which characters specify the mode in which the file stream
            ...
            :class:`~flib.errors.PullError`. If mode contains the character
            'w', then a :meth:`.push`-request is executed when closing the
            FileProxy instance with :meth:`.close`, otherwise any push-request
            raises a :class:`~flib.errors.PushError`. The default mode is 'rw'.
...
def openx(file: FileRef, *args: Any, **kwds: Any) -> Iterator[FileLike]:
    ...
    Args:
        file: :term:`File reference` that points to a valid filename in the
            ...
            instance of the class :class:`~flib.io.abc.Connector`.
        mode: String, which characters specify the mode in which the file stream
