...
import weakref
from flib.base import abc, attrib, literal
from flib.errors import FileFormatError, ColumnLookupError
from flib.io import plain, FileRef, FileRefClassInfo, FileConnector, FileProxy
from flib.typing import check, OptInt, OptIntTuple, OptStr, StrList
from flib.typing import IntTuple, OptList, OptStrTuple, Iterator
from flib.typing import ErrStack, ErrMeta, ErrType, StrDict, FileLike, StrTuple
...
class File(attrib.Group):
    ...
    _file: property = attrib.Temporary(dtype=FileRefClassInfo)
    ...
    def open(self, mode: str = 'r', columns: OptColumns = None) -> HandlerBase:
        ...
        Returns:
            ...
            of the class :class:`~flib.io.csv.Reader` is returned and in
            writing mode (if mode contains the character `r`) an instance of the
            class :class:`~flib.io.csv.Writer` is returned.
    ...
    def _get_header(self) -> StrTuple:
        ...
        if len(lines) != 1:
            raise FileFormatError(getattr(self._file, 'name', ''), 'CSV')
...
def load(
    ...
    Returns:
        Instance of class :class:`flib.io.csv.File`
